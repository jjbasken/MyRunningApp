...
class RunDetailViewModel @Inject constructor(
    ...
    val uiState = combine(repository.run(runId), repository.points(runId)) { run, points ->
        RunDetailUiState(false, run, points.map {
            RouteCoordinate(it.latitude, it.longitude, it.segmentIndex)
        })
